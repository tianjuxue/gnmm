...
def load_data():
    file_path = f'data/numpy/{args.shape_tag}/energy_sample_{args.num_samples}_resolution_{args.resolution}_{args.case_id}'
...
def jax_gpr_surrogate(train):
    ...
    y = train_data[:, -1]
    xtest = test_data[:, :-1]
    pickle_path = f"data/pickle/jax_gpr_resolution_{args.resolution}_{args.case_id}.pkl" 
...
def safe_batch_forward(batch_forward):
    rot_bound, disp_bound = np.load(args.path_bounds)
...
def show_contours():
    ...
    for i, disp in enumerate(np.linspace(-disp_bound, 0., 3)):
        ...
        plt.colorbar()
        contours = np.linspace(0, np.max(out), 11)
